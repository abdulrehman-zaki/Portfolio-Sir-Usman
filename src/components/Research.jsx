...
const Research = () => {
  ...
  return (
   <div>
          <h2 className="max-w-6xl mx-auto px-6 py-12 text-3xl font-bold mb-8 text-indigo-600">Research Papers</h2>
     <section id="research" className="max-w-6xl mx-auto px-2 py-16 bg-gray-50">
      <ul className="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-6">
        {papers.map((paper, i) => (
          <li key={i} className="bg-white p-5 rounded-lg shadow-md">
            <h3 className="font-semibold text-lg">{paper.title}</h3>
            <p className="text-gray-600">{paper.journal}</p>
            <a href={paper.link} className="text-indigo-600 hover:underline">
              View Paper →
            </a>
          </li>
        ))}
